<div class="flex flex-col md:flex-row gap-4">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-800">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">{{ \Carbon\Carbon::parse($bibit->tanggal)->format('d M Y') }}</td>
                    <td class="px-6 py-4">{{ number_format($bibit->kuantitas) }}</td>
                    <td class="px-6 py-4">{{ $bibit->type }}</td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end space-x-3">
                            ...
                            <form action="{{ route('bibit.destroy', $bibit->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="5" class="text-center py-4 text-gray-500">Belum ada data bibit.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="w-full md:w-64 bg-[#696CFF] text-white p-4 rounded-lg shadow-md">
        <h3 class="text-lg font-semibold mb-4">Total Bibit</h3>
        <div class="space-y-3">
            <div>
                <label class="block text-sm font-medium mb-1">Total (kg)</label>
                <div class="bg-white/20 p-2 rounded text-xl font-bold">
